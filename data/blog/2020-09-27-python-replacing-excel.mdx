...
date: 2020-09-27
summary:
  I read another blog post recently claiming the rise of Python would spell
...
I liked this way of looking at things. Even though it wasn't an original insight, it made sense to me as someone who has spent years working with a technology with an unofficial slogan of "Escape Excel Hell" for decades. Yet, in 2020, there's still quite a lot of spreadhseet misery going around.
...
When I read articles about this Python-based revolution, most seem to focus on comparing Pandas to Excel for manipulating and analysing data. Leaving aside the difficulty in learning a new tool, this feels like comparing apples to oranges.
...
Some pundits seem to think that there is a whole new generation entering the workforce that has learned Python. Some will, but a lot more will have spent just enough time on Stack Overflow to submit an assignment. For those in entry-level roles, I'd suggest even the latter is going to be rare.
...
Some of the use cases I've seen labelled as "Python is better than Excel" aren't talking about that at all. Instead, they're describing the process of transitioning to using a completely different way of modelling.

I've heard people comparing the use of a rudimentary Excel model with a few assumptions to do a rolling forecast to a fully-fledged scikit learn model trained with years of historical data. Of course, Python is a better choice here but you're not talking about the same thing.
