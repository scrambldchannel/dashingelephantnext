...
date: 2020-09-28
summary: This is a list of common git commands I tend to use, aimed at a relative newbie. I actually used it to give an intro to git presentation to my team a few years ago but it's still a nice reminder of basic usage. I'll tweak it as I go along.
tags:
  - git
  - github
  - bash
...
## Git snippets worth remembering
...
This is fine, but I've lost count of the times I've created the branch but forgotten to check it out. This has sometimes led me to commit changes to the wrong branch and have to clean up the mess. So I generally use this command to do both steps in a single line:
...
This will commit all staged changes to the repository with the commit message you provide. Note, this is the point at which it can be worth taking a deep breath and checking that you're doing what you want.
...
Once you've committed some changes to a branch, you may want to add these changes to another branch. For example, I might want to merge the changes made in features/my_new_feature into my master branch:
...
When working on a project hosted remotely (e.g. on Github) you will eventually want to push your changes up to the remote repository. If the branch you're working on already exists, you can push changes with this command:
...
When changes have been made upstream, I fetch changes and rebase my local master branch before pushing it to my remote fork. Changes can be fetched from upstream with this command:

```bash
git fetch upstream
```

### Rebase a local branch on a remote
...
Sometimes, someone will create a new branch in the upstream repository that I wish to clone locally. Or perhaps I want to check out a branch I've pushed to my remote fork repository from a different development environment.
